...
import Progrerss from "./account/Progress";
...
const OrderSuccess = () => {
  ...
  const res = router.query.state;
  const cartTotal = cartContext.cartTotal;
  const curContext = useContext(CurrencyContext);
  const symbol = curContext.state.symbol;
  const id = cartContext.state.items;

  console.log(res, "items from state");
  return (
    <CommonLayout parent="home" title="order success">
      <Progrerss curren={3} />
      <section className="section-b-space light-layout white-1 mt-5">
        <Container>
          <Row>
            <Col md="12">
